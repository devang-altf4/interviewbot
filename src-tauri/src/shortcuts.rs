    ...
    #[cfg(target_os = "windows")]
    {
        ...
        *is_hidden = !*is_hidden;
        ...
        if !*is_hidden {
            ...
            // Re-apply ghost mode after showing to ensure taskbar icon stays hidden
            crate::window::enforce_ghost_mode(&window);
...
fn handle_audio_shortcut<R: Runtime>(app: &AppHandle<R>) {
    if let Some(window) = app.get_webview_window("main") {
        ...
        if let Ok(false) = window.is_visible() {
            ...
            // Re-apply ghost mode after showing on Windows
            #[cfg(target_os = "windows")]
            crate::window::enforce_ghost_mode(&window);
...
pub fn set_app_icon_visibility<R: Runtime>(app: AppHandle<R>, visible: bool) -> Result<(), String> {
    ...
    #[cfg(target_os = "windows")]
    {
        // On Windows, use Win32 API to properly control taskbar icon visibility
        ...
        if let Some(window) = app.get_webview_window("main") {
            let raw_hwnd = window.hwnd().map_err(|e| format!("Failed to get window handle: {}", e))?.0 as *mut std::ffi::c_void;
            ...
            unsafe {
                ...
                // CRITICAL: Call SetWindowPos with SWP_FRAMECHANGED to apply the style changes
                // Use SWP_NOZORDER to preserve the current z-order (don't affect always-on-top state)
                let _ = SetWindowPos(
                    hwnd,
