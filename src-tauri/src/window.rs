...
/// Enforces ghost mode on Windows - hides the window from the taskbar
/// This should be called after showing a window to ensure it stays hidden from taskbar
#[cfg(target_os = "windows")]
pub fn enforce_ghost_mode<R: Runtime>(window: &WebviewWindow<R>) {
    ...
    if let Ok(hwnd_raw) = window.hwnd() {
        let raw_hwnd = hwnd_raw.0 as *mut std::ffi::c_void;
        let hwnd = HWND(raw_hwnd);

        unsafe {
            // Set extended styles to hide from taskbar
            ...
            // CRITICAL: Force Windows to re-evaluate the window styles
            let _ = SetWindowPos(
                hwnd,
                HWND::default(),
                0, 0, 0, 0,
